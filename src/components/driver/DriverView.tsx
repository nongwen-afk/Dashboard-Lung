...
import { getAllDepartures } from "@/mock-data/timetables";
import { getDriverForTrip } from "@/lib/shiftRotation";
import { LogOut, MapPin, Clock, Bus, CheckCircle2, AlertCircle, Calendar as CalendarIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
export function DriverView() {
  ...
  let nextDepartureInfo: { minutes: number; seconds: number; time: string; tripIndex: number } | null = null;
  if (isToday && nextTrip) {
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-y-auto pb-12 font-sans">
        ...
        <header className="max-w-2xl mx-auto px-6 py-8 pb-10 relative z-10">
            ...
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full border-4 border-white shadow-[0_0_20px_rgba(0,0,0,0.05)] flex items-center justify-center overflow-hidden">
                {currentUser.avatar ? (
                  ...
                  <span className="text-2xl font-bold text-gray-500">{currentUser.name.charAt(0)}</span>
                )}
              </div>
              <div>
                <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{currentUser.name} {currentUser.surname}</h1>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="bg-gray-50 rounded-3xl p-4 border border-gray-100 flex items-center gap-3 transition-transform duration-300 hover:scale-[1.02]">
              <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-inner" style={{ backgroundColor: routeDetails?.bgColor, color: routeDetails?.color }}>
                <MapPin className="w-5 h-5" />
              </div>
              <div>
                <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">เส้นทาง (Route)</p>
                <p className="font-bold text-sm text-gray-900 leading-tight">{routeDetails?.labelTh || currentUser.route}</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-3xl p-4 border border-gray-100 flex items-center gap-3 transition-transform duration-300 hover:scale-[1.02]">
              ...
              <div>
                <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">รถประจำ (Vehicle)</p>
                <p className="font-bold text-sm text-gray-900 leading-tight">{currentUser.vehicle}</p>
              </div>
            </div>
          </div>
        </header>
      ...
      <main className="flex-1 max-w-2xl w-full mx-auto px-4 mt-6 space-y-6">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-3 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-8 flex items-center justify-between border border-white/40 ring-1 ring-gray-900/5">
          <button onClick={handlePrevDay} className="p-3 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full transition-colors">
            <ChevronLeft className="w-5 h-5" />
          </button>
          
          <div className="flex flex-col items-center flex-1 cursor-pointer group" onClick={handleToday} role="button">
            ...
            {!isToday && <span className="text-[10px] text-blue-500/70 font-medium uppercase tracking-widest mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">แตะเพื่อกลับไปวันนี้</span>}
          </div>

          <button onClick={handleNextDay} className="p-3 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full transition-colors">
              ...
              {nextDepartureInfo ? (
                <div className="flex items-end justify-between">
                  <div>
                    ...
                    <p className="text-[11px] font-medium text-gray-400 uppercase tracking-widest">เวลาออกรถ (Departure)</p>
                  </div>
                  <div className="text-right">
                    <div className="flex items-baseline gap-1.5 text-green-400 justify-end mb-1">
                      <span className="text-4xl font-bold tracking-tight shadow-green-400/20">{nextDepartureInfo.minutes}</span>
                      <span className="text-sm font-semibold">นาที</span>
                    </div>
                    <p className="text-[11px] font-medium text-gray-400 uppercase tracking-widest">นับถอยหลัง</p>
                  </div>
                </div>
              ) : (
                <div className="py-6 text-center">
                  ...
                  <p className="text-sm text-gray-400 mt-1 font-medium">พักผ่อนให้เพียงพอ เจอกันพรุ่งนี้ครับ</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
          ...
          {departuresList.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-1">
                      <div>
                        ...
                        <p className="text-[11px] font-semibold text-gray-400 mt-1.5">รอบที่ {dept.tripIndex + 1}</p>
                      </div>

                      {/* Route Details and Info */}
                      <div className="flex flex-wrap items-center gap-4 sm:text-right">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-gray-700">ม.มหิดล ศาลายา → {destName}</span>
                          <span className="text-[10px] text-gray-400 mt-0.5">รถประจำ: {currentUser.vehicle}</span>
                        </div>
            ...
            <div className="text-center py-12 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-500 text-sm font-medium">ไม่มีรอบวิ่ง{isToday ? "เหลือในวันนี้" : "ในวันนี้"}</p>
            </div>
          )}
        </div>
      </main>
