...
export function BusCard({ driver, routeColor, expanded }: BusCardProps) {
      ...
      <div
        ...
        onMouseEnter={(e) => {
          if (!isLeave) {
            ...
            el.style.background = isSubstitute
              ? `${routeColor}20`
              : `linear-gradient(135deg, rgba(255,255,255,1), ${routeColor}06)`;
        ...
        onMouseLeave={(e) => {
          if (!isLeave) {
            ...
            el.style.boxShadow = isSubstitute
              ? `0 1px 4px ${routeColor}10`
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        if (!isLeave) {
          ...
          el.style.boxShadow = isSubstitute
            ? `0 1px 4px ${routeColor}10`
            : "0 2px 6px rgba(26,26,46,0.07), inset 0 1px 0 rgba(255,255,255,1)";
          el.style.border = isSubstitute
            ? `1px solid ${routeColor}30`
    ...
    >
      ...
      />
      <p className="text-[0.78rem] text-center text-gray-500 truncate max-w-[4.8rem] mx-auto relative z-10">
        {driver.name}
      </p>
      <p className="text-[0.66rem] text-center text-gray-400 relative z-10">{driver.vehicle}</p>
      {isLeave && (
        <p
          className="text-[0.66rem] text-center font-bold relative z-10"
          style={{ color: "#dc2626" }}
        >
          LEAVE
        </p>
      ...
      {isSubstitute && (
        <p
          className="text-[0.66rem] text-center font-bold relative z-10 mt-0.5"
