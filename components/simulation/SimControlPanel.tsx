  ...
  return (
    <div className="mb-4">
      ...
      <div className="relative h-2.5 rounded-full bg-slate-200">
        ...
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-5 h-5 rounded-full border-2 border-white pointer-events-none shadow-sm"
...
function Toggle({
  ...
  return (
    ...
    >
      ...
      <div
        className="w-11 h-6 rounded-full relative shadow-inner flex-shrink-0 transition-colors duration-300"
      ...
      {options.map((opt) => (
        <button
          ...
          style={
            value === opt.value
              ? ({
                  borderColor: accent || "#6366f1",
                  "--tw-ring-color": accent || "#6366f1",
        ...
        >
          <div>
            <p
              className={`text-sm font-bold ${value === opt.value ? "text-slate-800" : "text-slate-600"}`}
            >
              {opt.label}
            </p>
          ...
          <div
            className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${value === opt.value ? "" : "border-slate-300"}`}
            style={value === opt.value ? { borderColor: accent || "#6366f1" } : {}}
          >
            {value === opt.value && (
              <div className="w-2 h-2 rounded-full" style={{ background: accent || "#6366f1" }} />
            )}
          </div>
        </button>
      ))}
...
export function SimControlPanel({ config, multiResult, onChange, onReset }: Props) {
  ...
  const breakModeOptions: SegmentOption<BreakMode>[] = [
    {
      ...
      hint: "(Greedy Algorithm) จัดสรรเวลาพักตามความเหมาะสม",
    },
  ...
  const fairnessModeOptions: SegmentOption<FairnessMode>[] = [
    {
      value: "fewest-trips",
      label: "รอบวิ่งน้อยที่สุดก่อน",
      hint: "จัดสรรให้ผู้ขับขี่ที่มีรอบสะสมน้อยที่สุด",
    },
    {
      value: "round-robin",
      label: "ตามลำดับ (Round-Robin)",
      hint: "จัดสรรคิวตามลำดับรายชื่อที่กำหนด",
    },
    {
      value: "earliest-free",
      label: "พร้อมปฏิบัติงานก่อน",
      hint: "จัดสรรให้ผู้ขับขี่ที่พร้อมปฏิบัติงานเป็นลำดับแรก",
    },
  ];
    ...
    <div className="w-full h-full flex flex-col overflow-hidden bg-white">
      ...
      <div className="flex-shrink-0 p-4 border-b border-slate-200 bg-white z-10 shadow-sm">
        <div className="grid grid-cols-2 gap-2">
          ...
          <div
            className={`border rounded-xl p-3 text-center shadow-sm transition-colors ${totalOverlap > 0 ? "bg-red-50 border-red-200" : "bg-slate-50 border-slate-200"}`}
          >
            <p
              className={`text-xs font-semibold mb-1 ${totalOverlap > 0 ? "text-red-600" : "text-slate-500"}`}
            >
              Overlap
            </p>
            <p
              className={`text-lg font-black ${totalOverlap > 0 ? "text-red-700" : "text-slate-400"}`}
            >
              {totalOverlap} รอบ
            </p>
          </div>
      ...
      >
        ...
        {config.breakMode !== "sheet" && (
          <Slider
            ...
            hint={
              config.breakMode === "smart"
                ? "เวลาสูงสุดที่ห้ามทำงานต่อเนื่อง"
        ...
        <div className="space-y-1.5">
          ...
          {allDriverWorkloads.length === 0 && (
            <p className="text-[0.65rem] text-slate-400 text-center italic py-2">
              ยังไม่มีผู้ขับขี่ถูกจัดสรรเส้นทาง
            </p>
          )}
        </div>
      </div>
