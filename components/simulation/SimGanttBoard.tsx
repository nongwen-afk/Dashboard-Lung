...
function minToPx(m: number) {
  return (m - GANTT_START) * PX_PER_MIN;
}
...
function computeLayout(
  ...
  const routes =
    activeView === "all" ? (["green", "blue", "red"] as RouteKey[]) : [activeView as RouteKey];

  // Calculate total drivers to divide space evenly
  let totalDrivers = 0;
  for (const route of routes) {
    totalDrivers += Math.max(
      1,
      multiResult[route].drivers.filter((d) => d.trips.length > 0).length
    );
  }
...
export function SimGanttBoard({ multiResult, tripDurations, otThresholdHours, activeView }: Props) {
  ...
  const { groups, totalWidth } = useMemo(
    () => computeLayout(multiResult, activeView, containerWidth),
    [multiResult, activeView, containerWidth]
  );
  const otThresholdMin = otThresholdHours * 60;

  const showTripTooltip = useCallback(
    (trip: TripAssignment, driver: DriverShift, route: RouteKey, e: React.MouseEvent) => {
  ...
  const showBreakTooltip = useCallback(
    (brk: BreakEvent, driver: DriverShift, route: RouteKey, e: React.MouseEvent) => {
  ...
  return (
    <div ref={containerRef} className="w-full h-full overflow-auto relative select-none bg-white">
              ...
              <div className="flex" style={{ height: 44 }}>
                {drivers.length === 0 ? (
                  ...
                  drivers.map((driver) => (
                    ...
                    >
                      <p
                        className="text-[0.65rem] font-bold text-slate-700 leading-none w-full text-center truncate px-1"
                        title={driver.name}
                      >
                        {driver.name}
                      </p>
      ...
      >
          ...
          return (
            <div
              ...
              style={{
                ...
                borderTop: "1px dashed rgba(239,68,68,0.18)",
                borderBottom: "1px dashed rgba(239,68,68,0.18)",
                zIndex: 1,
              }}
            >
              <span
                className="absolute right-2 top-1 text-[0.4rem] font-black uppercase tracking-widest"
        ...
        {HOUR_MARKS.map((h) => {
          ...
          return (
            <div
              key={h}
              className="absolute pointer-events-none"
              style={{ left: 0, right: 0, top, zIndex: 2 }}
            >
              <div
                className="absolute"
                style={{
                  ...
                  right: 0,
                  height: 1,
                  background: h % 2 === 0 ? "#cbd5e1" : "#f1f5f9",
                }}
              />
              {/* Sticky-left time label */}
              <div
                className="absolute flex items-center justify-end pr-2 bg-slate-50"
                style={{ left: 0, width: TIME_COL_W, top: -9, height: 18 }}
              >
                <span
                  className="text-[0.5rem] font-bold tabular-nums"
                  style={{ color: h % 2 === 0 ? "#475569" : "#94a3b8" }}
                >
              ...
              {drivers.map((driver, di) => {
                ...
                return (
                  <div
                    ...
                    style={{ left: colLeft, width: colW, zIndex: 3 }}
                  >
                    {/* Alternating col bg */}
                    <div
                      className="absolute inset-0 pointer-events-none"
                      style={{
                        background: di % 2 === 0 ? "transparent" : "rgba(0,0,0,0.02)",
                    ...
                    {driver.trips.length > 0 && otStartMin < driver.endMin && (
                      <div
                        ...
                        style={{
                          left: 4,
                          right: 4,
                          top: minToPx(otStartMin),
                          height: Math.max(0, (driver.endMin - otStartMin) * PX_PER_MIN),
                          background: "rgba(249,115,22,0.07)",
                          borderTop: "1.5px dashed rgba(249,115,22,0.5)",
                          borderRadius: "0 0 4px 4px",
                          zIndex: 0,
                        }}
                      >
                        <span
                          className="absolute top-0.5 left-1 text-[0.4rem] font-black"
                    ...
                    {driver.trips.length > 0 && (
                      <div
                        ...
                        style={{
                          left: 5,
                          right: 5,
                          top: minToPx(driver.startMin),
                          height: (driver.endMin - driver.startMin) * PX_PER_MIN,
                          background: `${meta.color}08`,
                          borderLeft: `1.5px solid ${meta.color}20`,
                          borderRight: `1.5px solid ${meta.color}20`,
                        }}
                      />
                    )}
                    ...
                    {driver.trips.map((trip) => {
                      ...
                      return (
                        ...
                        >
                          {h > 14 && (
                            <span
                              className="text-[0.4rem] font-black text-white drop-shadow-sm text-center px-0.5"
                              style={{ textShadow: "0 1px 2px rgba(0,0,0,0.6)", lineHeight: 1.2 }}
                            >
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: "6px",
                              boxShadow:
                                "0 4px 10px rgba(250,204,21,0.3), inset 0 1px 0 rgba(255,255,255,0.8)",
                              display: "flex",
                              alignItems: "center",
                              justifyContent: "center",
                              overflow: "hidden",
                          ...
                          >
                            <span
                              className="text-[0.4rem] font-black text-center px-0.5"
                              style={{ color: "#78350f", lineHeight: 1.2 }}
                            >
                              พัก{"\n"}30 นาที
                            </span>
                          </div>
                    ...
                    {/* Idle gap indicators */}
                    {showIdle &&
                      driver.trips.length > 1 &&
                      driver.trips.slice(0, -1).map((trip, ti) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              height: h,
                              background:
                                "repeating-linear-gradient(45deg,transparent,transparent 3px,rgba(148,163,184,0.12) 3px,rgba(148,163,184,0.12) 6px)",
                          ...
                          >
                            {h > 20 && (
                              <span className="text-[0.35rem] text-slate-400 font-bold">
                                รอ {idleDur}น.
                              </span>
                            )}
                          </div>
                    ...
                    {/* OT start line */}
                    {driver.trips.length > 0 &&
                      otStartMin >= driver.startMin &&
                      otStartMin <= driver.endMin && (
                        <div
                          className="absolute pointer-events-none"
                          style={{
                            left: 0,
        ...
        {(() => {
          ...
          return (
            ...
            >
              <div
                className="absolute -left-1.5 -top-1 w-3 h-3 rounded-full bg-cyan-400"
      ...
      {/* ── Legend + filter bar ── */}
      <div className="sticky bottom-0 flex items-center gap-3 px-4 py-1.5 border-t flex-wrap z-30 bg-white border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        {/* Route coverage badges */}
        {(["green", "blue", "red"] as RouteKey[])
          .filter((r) => activeView === "all" || activeView === r)
          .map((r) => (
            <div key={r} className="flex items-center gap-1">
              <div className="w-4 h-3 rounded-sm" style={{ background: ROUTE_META[r].color }} />
              <span className="text-[0.45rem] text-slate-600 font-bold">
                {ROUTE_META[r].label} ({multiResult[r].coverageRate.toFixed(0)}%)
              </span>
            </div>
          ))}
        ...
        {/* Filter toggles */}
        <span className="text-[0.45rem] text-slate-400 font-bold uppercase tracking-wide">
          แสดง:
        </span>
        {(
          [
        ...
        <div className="ml-auto text-[0.45rem] text-slate-500 font-bold">
          เลื่อน ↕↔ เพื่อดูทั้งหมด
        </div>
      </div>
        ...
        >
            ...
            <>
              <div className="flex items-center gap-2 mb-1.5">
                <div
                  ...
                  style={{
                    background: tooltip.trip.isOT ? "#f59e0b" : ROUTE_META[tooltip.route].color,
                  }}
                />
                <p className="text-[0.625rem] font-black text-slate-800">{tooltip.driver.name}</p>
                <span
                  className="text-[0.45rem] px-1.5 py-0.5 rounded-full font-bold"
                  style={{
                    background: `${ROUTE_META[tooltip.route].color}22`,
                    color: ROUTE_META[tooltip.route].color,
                  }}
                >
                  {ROUTE_META[tooltip.route].label}
                </span>
              </div>
              <p className="text-[0.5625rem] text-slate-500 font-bold">
                รอบที่ {tooltip.trip.tripIndex + 1}
              </p>
              <p className="text-[0.5625rem] text-slate-600 mt-0.5">
                {minToTime(tooltip.trip.departureMin)} →{" "}
                {minToTime(tooltip.trip.departureMin + tripDurations[tooltip.route])}
              </p>
              <div className="flex gap-1.5 mt-1.5 flex-wrap">
                {tooltip.trip.isOverlapping && (
                  <span
                    className="px-1.5 py-0.5 rounded text-[0.45rem] font-black"
                    style={{
          ...
          {tooltip.kind === "break" && (
            <>
              ...
              <p className="text-[0.5625rem] text-slate-600 mt-0.5">
                {minToTime(tooltip.brk.startMin)} → {minToTime(tooltip.brk.endMin)}
              </p>
            </>
          )}
        </div>
