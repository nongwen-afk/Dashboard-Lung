...
import { ROUTE_META, ALL_DRIVERS, type AssignmentMap, type RouteKey } from "@/lib/simulationEngine";
...
export function DriverAssignmentBoard({ assignments, onChange }: Props) {
  ...
  const assignDriver = useCallback(
    (name: string, target: RouteKey | "pool") => {
  ...
  const onDrop = useCallback(
    (e: React.DragEvent, zoneKey: RouteKey | "pool") => {
      e.preventDefault();
      if (draggedDriver) {
    ...
    <div className="flex-shrink-0 border-b border-slate-200 bg-white">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 bg-slate-50">
        <div className="flex items-center gap-2">
          <span className="text-[0.5625rem] font-black uppercase tracking-widest text-slate-700">
            👥 จัดสรรคนขับ
          </span>
            ...
            >
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[0.5625rem] font-black" style={{ color: zone.color }}>
                    {zone.label}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  {deficit > 0 && (
                    <span className="text-[0.45rem] font-bold text-amber-600 bg-amber-100 px-1 rounded-sm border border-amber-200">
                      -{deficit}
                    </span>
                  )}
                  {excess > 0 && (
                    <span className="text-[0.45rem] font-bold text-cyan-700 bg-cyan-100 px-1 rounded-sm border border-cyan-200">
                      +{excess}
                    </span>
                  )}
                </div>
