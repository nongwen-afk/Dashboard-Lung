...
export const ROUTE_META: Record<
  RouteKey,
  { label: string; color: string; bgColor: string; defaultDrivers: number; defaultOT: number }
> = {
  green: {
    label: "สายสีเขียว",
    color: "#16a34a",
...
export function isRushHour(minFromMidnight: number): boolean {
  ...
  return (
    (minFromMidnight >= hm(6, 30) && minFromMidnight <= hm(9, 0)) ||
...
export function runSimulation(config: SimConfig): SimResult {
  ...
  const regularDriverNames =
    config.customDriverNames && config.customDriverNames.length > 0
      ...
      for (let d = 0; d < driverNames.length; d++) {
        if (driverWorkStart[d] !== -1) {
          ...
          if (dep >= limit) {
            ...
            const breakStart =
              breakMode === "cumulative" ? limit : Math.max(driverAvailableAt[d], limit);
  ...
  const drivers: DriverShift[] = driverNames
    .map((name, d) => {
  ...
  const avgWorkHours =
    drivers.length > 0 ? drivers.reduce((s, d) => s + d.workMinutes, 0) / drivers.length / 60 : 0;
  const avgTripsPerDriver =
    drivers.length > 0 ? drivers.reduce((s, d) => s + d.trips.length, 0) / drivers.length : 0;
...
export function runAllRoutes(
  ...
  if (shared.rotateRoutes && shared.simulationDay > 1 && allActiveDrivers.length > 0) {
    ...
    const rotatedGlobal = [
      ...allActiveDrivers.slice(globalOffset),
      ...allActiveDrivers.slice(0, globalOffset),
    ];
  ...
  const makeResult = (route: RouteKey): SimResult => {
    ...
    if (customDriverNames.length > 0 && shared.simulationDay > 1 && shared.enableDayOff) {
      ...
      customDriverNames = [
        ...customDriverNames.slice(offset),
        ...customDriverNames.slice(0, offset),
      ];
    }
    ...
    if (customDriverNames.length === 0) {
      ...
      return {
        ...
        departures: shared.customDepartures
          ? shared.customDepartures[route][shared.dayType]
          : ROUTE_DEPARTURES[route][shared.dayType],
        drivers: [],
        totalTrips: shared.customDepartures
          ? shared.customDepartures[route][shared.dayType].length
          : ROUTE_DEPARTURES[route][shared.dayType].length,
        totalDrivers: 0,
        avgWorkHours: 0,
        avgTripsPerDriver: 0,
        totalOTCount: 0,
    ...
    return runSimulation({
      ...
      customDriverNames,
      assistDriverNames:
        shared.crossLineAssist && route === "blue" ? ["(ช่วย) สายเขียว"] : undefined,
